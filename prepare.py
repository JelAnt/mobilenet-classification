...
splitsize = 0.85 # variable used to specify that 85% of images will be used for training
...
folders = os.listdir(source_folder) # creating a list of folders inside of the dataset folder


for folder in folders: #for each folder inside the dataset folder append the folder name to the category list
...
def split_data(SOURCE, training, validation, split_size): # function that will split the dataset into training and validation data
    ...
    for filename in os.listdir(SOURCE): 
        ...
        if os.path.getsize(file) > 0: 
            files.append(filename) # if the file doesn't already exist in the list add it to it
        else:
            print(filename + " empty")

    training_length = int(len(files) * split_size) # specify the length of the training data
    shuffleSet = random.sample(files, len(files)) # shuffle all the files
    trainingSet = shuffleSet[0:training_length] # specifies the files that belong to the training set
    validSet = shuffleSet[training_length:] # the rest of the files belong to the validation set
...
validatePath = target_folder + "/validate/" # specifying paths of training and validation folders
...
for category in categories: # create an appropriate folder for each category in the list
    ...
    split_data(sourcePath, trainDestPath, validateDestPath, splitsize) # run the splitdata function. (finish preparing the dataset)
